    ...
    return (
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        ...
        <ThemedView style={styles.container}>
          <ThemedText style={styles.title} type="title">{t('auth.loginTitle')}</ThemedText>
            ...
            {({ handleChange, handleBlur, handleSubmit, values, errors, touched }) => (
              <View style={styles.form}>
                ...
                <TouchableOpacity style={styles.button} disabled={isLoading}  onPress={() => handleSubmit()}>
                  {isLoading ? <ActivityIndicator color="#fff" /> : <ThemedText style={styles.buttonText}>{t('auth.login')}</ThemedText>}
                {/* <ThemedText style={styles.buttonText}>{t('auth.login')}</ThemedText> */}
                </TouchableOpacity>

                {/* <ThemedText style={styles.bottomText}>
                {t("auth.noAccount")} <ThemedText style={styles.link}>{t("auth.register")}</ThemedText>
                </ThemedText> */}

                <ThemedText style={styles.bottomText}>
                  {t("auth.noAccount")}{" "}
                    <ThemedText onPress={() => router.push('/register')} style={styles.link}>{t("auth.register")}</ThemedText>
                  {/* <TouchableHighlight onPress={() => router.push('/register')}>
                  </TouchableHighlight> */}
                </ThemedText>
              </View>
            )}
